...
def loadMovieNames():
    ...
    with open("movies.dat", encoding="ISO-8859-1") as f:
        for line in f:
            fields = line.split("::")
            movieNames[int(fields[0])] = fields[1].encode().decode('ISO-8859-1', 'ignore')
    return movieNames
...
if (len(sys.argv) > 1):
    ...
    filteredResults = moviePairSimilarities.filter(lambda pair__sim: \
        (pair[0] == movieID or pair[1] == movieID) \
        and (sim[0] > scoreThreshold and sim[1] > coOccurenceThreshold))

    # Sort by quality score.
    results = filteredResults.map(lambda pair__sim: (sim, pair)).sortByKey(ascending = False).take(10)
