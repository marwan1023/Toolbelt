...
// Class MuliScheduler does the same as specified for Scheduler
public class MultiScheduler extends Thread {
    ...
    private int level = 1; // Queue level(i.e. 0=SJF, 1 = RR, or 2=FCFS)
    private TestThread currentThread; // Current running thread
    private int order = 0;

    // Initialization on the data members
    public MultiScheduler() {
    ...
    public MultiScheduler(int quantum) {
    ...
    // Enters the testThread intance into the level one queue. The level one
    // queue
