...
PREPARE_LOGGING(HelloWorld_i)

HelloWorld_i::HelloWorld_i(const char *uuid, const char *label) :
    HelloWorld_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

HelloWorld_i::~HelloWorld_i()
{
}

void HelloWorld_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void HelloWorld_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &HelloWorld_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &HelloWorld_i::<callback method>)
        in the constructor.
        ...
        //Add to HelloWorld.cpp
        HelloWorld_i::HelloWorld_i(const char *uuid, const char *label) :
            HelloWorld_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &HelloWorld_i::scaleChanged);
            addPropertyListener(status, this, &HelloWorld_i::statusChanged);
        }

        void HelloWorld_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void HelloWorld_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
...
************************************************************************************************/
int HelloWorld_i::serviceFunction()
{
