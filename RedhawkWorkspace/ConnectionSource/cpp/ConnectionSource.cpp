...
PREPARE_LOGGING(ConnectionSource_i)

ConnectionSource_i::ConnectionSource_i(const char *uuid, const char *label) :
		ConnectionSource_base(uuid, label) {
	// Avoid placing constructor code here. Instead, use the "constructor" function.

}

ConnectionSource_i::~ConnectionSource_i() {
}

void ConnectionSource_i::constructor() {
	/***********************************************************************************
...
/***********************************************************************************************
 ...
 void ConnectionSource_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
 }

 Register the message callback onto the input port with the following form:
 this->msg_input->registerMessage("my_msg", this, &ConnectionSource_i::my_message_callback);
 ...
 addPropertyListener(<property>, this, &ConnectionSource_i::<callback method>)
 in the constructor.
 ...
 //Add to ConnectionSource.cpp
 ConnectionSource_i::ConnectionSource_i(const char *uuid, const char *label) :
 ConnectionSource_base(uuid, label)
 {
 addPropertyListener(scaleValue, this, &ConnectionSource_i::scaleChanged);
 addPropertyListener(status, this, &ConnectionSource_i::statusChanged);
 }

 void ConnectionSource_i::scaleChanged(float oldValue, float newValue)
 {
 RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
 }

 void ConnectionSource_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
 {
 RH_DEBUG(this->_baseLog, "status changed");
 }

 //Add to ConnectionSource.h
 ...
 ************************************************************************************************/
int ConnectionSource_i::serviceFunction() {
	if (data.size() != this->xfer_length) {
		data.resize(xfer_length);
	}
	if (!stream) {
		//short stream_id = 1;
		//bulkio::OutShortStream sri = bulkio::NumericOutputStream<short>::sri::
		//BULKIO::dataShort sample = new BULKIO::dataShort("sample");
