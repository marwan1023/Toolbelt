 const getSensorReadings = require('./get-sensor-readings') /*    Instantiate the cache. In this case its a simple variable     stored in local memory    */
 const cache = {
     temperature: null,
     humidity: null
 }
 ...
 setInterval(() => {
     getSensorReadings((err, temperature, humidity) => {
         ...
         cache.temperature = temperature cache.humidity = humidity
     })
 }, 2000)

 /*    The functions that we expose only return the cached     values, and don't make a call to the sensor interface     everytime    */
 module.exports.getTemperature = () => cache.temperature module.exports.getHumidity = () => cache.humidity